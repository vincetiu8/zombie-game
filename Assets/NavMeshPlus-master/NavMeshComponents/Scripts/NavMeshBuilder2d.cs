...
namespace UnityEngine.AI
{
    ...
    internal class NavMeshBuilder2d
    {
        internal static void CollectSources(List<NavMeshBuildSource> sources, NavMeshBuilder2dWrapper builder)
        {
            ...
            foreach (GameObject it in root) CollectSources(it, sources, builder);
        }

        private static void CollectSources(GameObject root, List<NavMeshBuildSource> sources,
            NavMeshBuilder2dWrapper builder)
        {
                ...
                if (builder.defaultArea != 1 && !modifier.ignoreFromBuild)
                {
                    ...
                    if (tilemap != null)
                    {
                        ...
                        NavMeshBuildSource box =
                            BoxBoundSource(NavMeshSurface2d.GetWorldBounds(tilemap.transform.localToWorldMatrix,
                                tilemap.localBounds));
        ...
        private static void CollectSources(List<NavMeshBuildSource> sources, SpriteRenderer sprite, int area,
            NavMeshBuilder2dWrapper builder)
        {
            ...
            src.transform = Matrix4x4.TRS(Vector3.Scale(sprite.transform.position, builder.overrideVector),
                sprite.transform.rotation, sprite.transform.lossyScale);
            src.sourceObject = mesh;
            sources.Add(src);
        }

        private static void CollectSources(List<NavMeshBuildSource> sources, NavMeshModifier modifier, int area,
            NavMeshBuilder2dWrapper builder)
        {
            ...
            if (collider.attachedRigidbody)
                src.transform = Matrix4x4.TRS(Vector3.Scale(collider.transform.position, builder.overrideVector),
        ...
        private static void CollectTileSources(List<NavMeshBuildSource> sources, Tilemap tilemap, int area,
            NavMeshBuilder2dWrapper builder)
        {
            ...
            for (int i = bound.xMin; i < bound.xMax; i++)
            {
                for (int j = bound.yMin; j < bound.yMax; j++)
                {
                    ...
                    if (!builder.overrideByGrid && tilemap.GetColliderType(vec3int) == Tile.ColliderType.Sprite)
                    {
                        ...
                        src.transform =
                            Matrix4x4.TRS(
                                ...
                                tilemap.layoutGrid.cellGap, tilemap.transform.rotation, tilemap.transform.lossyScale) *
                            tilemap.orientationMatrix * tilemap.GetTransformMatrix(vec3int);
                        src.sourceObject = mesh;
                        sources.Add(src);
                    }
                    ...
                    {
                        src.transform =
                            Matrix4x4.TRS(Vector3.Scale(tilemap.GetCellCenterWorld(vec3int), builder.overrideVector),
                                rot, size);
                    ...
                    {
                        NavMeshBuildSource boxsrc = new NavMeshBuildSource();
                        boxsrc.transform =
                            Matrix4x4.TRS(
                                Vector3.Scale(tilemap.GetCellCenterWorld(vec3int), builder.overrideVector) -
                                tilemap.layoutGrid.cellGap, tilemap.transform.rotation, tilemap.transform.lossyScale) *
