    ...
    public class NavMeshAssetManager2d : ScriptableSingleton<NavMeshAssetManager2d>
    {
        ...
        static string GetAndEnsureTargetPath(NavMeshSurface2d surface)
        {
            ...
            else
            {
                ...
                {
                    string prefabDirectoryName = Path.GetDirectoryName(prefabStage.assetPath);
        ...
        void StoreNavMeshDataIfInPrefab(NavMeshSurface2d surfaceToStore)
        {
                ...
                !PrefabUtility.IsPartOfModelPrefab(surfaceToStore))
            {
                var basePrefabSurface =
                    PrefabUtility.GetCorrespondingObjectFromSource(surfaceToStore) as NavMeshSurface2d;
            ...
            do
            {
                ...
                for (var i = m_PrefabNavMeshDataAssets.Count - 1; i >= 0; i--)
                {
                    ...
                    else if (surfaceInPrefab != null && storedPrefabInfo.surface == surfaceInPrefab)
                    {
                        ...
                        var baseSurface =
                            PrefabUtility.GetCorrespondingObjectFromSource(surfaceInPrefab) as NavMeshSurface2d;
